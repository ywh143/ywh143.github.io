...
public interface ArticleMapper {
    ...
    //查询博客展示分页文章信息，指定起始记录和步长。实现文章表和用户表连接查询
    @Select("select * from blog.article,blog.user where article.uid = user.user_id order by creat desc " +
    ...
    //查询博客管理分页文章信息，指定起始记录和步长。实现文章表和用户表连接查询
    @Select("select * from blog.article,blog.user where article.uid = user.user_id and article.uid = #{uidNum} order by creat desc " +
            "limit #{startNum},#{recordNum}")
    @Results({
            @Result(column = "id",property = "id"),
            @Result(column = "title",property = "title"),
    ...
    List<Article> findByArticleManage(int startNum,int recordNum,int uidNum);

    //查询分类文章信息，指定起始位置、步长和文章分类
    @Select("select * from blog.article where category = #{category} order by creat desc limit #{startNum},#{recordNum}")
    ...
    //根据作者id查询文章信息
    @Select("select * from blog.article,blog.user where article.uid = #{uidNum} and " +
            "article.uid = user.user_id limit #{startNum},#{recordNum}")
    @Results({
            @Result(column = "id",property = "id"),
            @Result(column = "title",property = "title"),
            @Result(column = "content",property = "content"),
    ...
    //获取文章行数
    @Select("select count(*) from blog.article")
    int countArticle();
}
